...
from Utilities.pibCopilot import indexDocs, createPressReleaseIndex, findEarningCalls, mergeDocs, createPibIndex, findPibData, deletePibData
...
def processStep2(pibIndexName, cik, step, symbol, llm, today, earningVectorIndexName, embeddingModelType, docs):
    r = findPibData(SearchService, SearchKey, pibIndexName, cik, step, returnFields=['id', 'symbol', 'cik', 'step', 'description', 'insertedDate',
                                                                   'pibData'])
    s2Data = []
    if r.get_count() == 0:
    ...
    try:
        ...
        if step == "1":
            ...
            indexEarningCallSections(OpenAiService, OpenAiKey, OpenAiVersion, OpenAiApiKey, SearchService, SearchKey,
                                    ...
                                    latestEarningsData['callDate'], latestEarningsData['symbol'], latestEarningsData['year'],
                                    latestEarningsData['quarter'])
            s2Data = processStep2(pibIndexName, cik, step, symbol, llm, today, earningVectorIndexName, embeddingModelType, docs)
            outputFinalAnswer = {"data_points": [content], "answer": s2Data, 
                            "thoughts": '',
                                "sources": '', "nextQuestions": '', "error": ""}
            return outputFinalAnswer
        elif step == "3":
